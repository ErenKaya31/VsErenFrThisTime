...
class SelectLanguageState extends MusicBeatState
{
	...
	public override function create()
	{
		...
		selectLanguage = new FlxText(110, (FlxG.height / 2) - 300, FlxG.width, "Pick a language", 45);
		selectLanguage.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		for (i in 0...langaugeList.length)
		{
			...
			var langaugeText:FlxText = new FlxText(110, (FlxG.height / 2 - 150) + i * 75, FlxG.width, currentLangauge.langaugeName, 25);
			//langaugeText.screenCenter(X);
			langaugeText.setFormat(Paths.font("vcr.ttf"), 25, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function updateText(text:FlxText, selected:Bool)
	{
		if (selected)
		{
			text.setFormat(Paths.font("vcr.ttf"), 25, FlxColor.YELLOW, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		}
		else
		{
			text.setFormat(Paths.font("vcr.ttf"), 25);
		}
	}
}
