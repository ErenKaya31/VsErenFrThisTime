...
class ChangeLanguageState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var helper:FlxText = new FlxText(110, 150, FlxG.width, "Pick a Language", 40);
		helper.setFormat(Paths.font("vcr.ttf"), 60, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		helper.antialiasing = !ClientPrefs.globalAntialiasing;
		helper.borderSize = 3;
		...
		for (i in 0...languages.length)
		{
			...
			var text:FlxText = new FlxText(110, 350 + (i * 75), FlxG.width, curLanguage.langaugeName, 40);
			text.setFormat(Paths.font("vcr.ttf"), 30, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE);
	...
	function changeTextState(text:FlxText, selected:Bool)
	{
		...
		{
			text.setFormat(Paths.font("vcr.ttf"), 40);
		}
	}
