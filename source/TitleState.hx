	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				FlxTween.tween(logoBl, {y: 2000}, 3, {ease: FlxEase.backInOut});
				FlxTween.tween(titleText, {y: 2000}, 3, {ease: FlxEase.backInOut});
				FlxTween.tween(gfDance, {y: 2000}, 3, {ease: FlxEase.backInOut});
				FlxTween.tween(gradientBar, {y: 2000}, 3, {ease: FlxEase.backInOut});
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				case 1:
					#if PSYCH_WATERMARKS
					createCoolText(['Vs Eren Kaya by'], 15);
					#else
		...
		if(curBeat % 4 == 0)
		{
			...
			FlxTween.tween(FlxG.camera, {zoom: FlxG.camera.zoom - 0.015}, 1);
		}
	}
